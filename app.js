...
// Exam Marks Modal
const closeExamModal = () => this.toggleModal('exam-marks-modal', false);
document.getElementById('exam-marks-modal')?.addEventListener('click', (e) => {
    if (e.target === document.getElementById('exam-marks-modal')) closeExamModal();
});
...
document.getElementById('exam-marks-form')?.addEventListener('submit', (e) => {
    e.preventDefault();
    const formData = new FormData(e.target);
    ...
    // Gather marks
    const subjects = ['Islamic', 'Arabic', 'Somali', 'Math', 'Science', 'Social', 'English', 'Geography', 'History', 'Physics', 'Biology'];
    const scores = [];

    subjects.forEach(sub => {
        const val = formData.get(`subject_${sub}`);
        if (val !== '') {
            scores.push({
                studentId,
                subject: sub,
                term,
                mark: parseFloat(val) || 0
            });
        }
    });
    ...
    if (scores.length > 0) {
        ...
        this.toggleModal('exam-marks-modal', false);
        this.showToast('Marks saved successfully!');
        // Refresh currently open student list or marks view logic
        // If we are in student list view, maybe just toast is enough, or refresh
    }
});

// User Management Listeners
const closeUserModal = () => this.toggleModal('add-user-modal', false);
document.getElementById('add-user-modal')?.addEventListener('click', (e) => {
    if (e.target === document.getElementById('add-user-modal')) closeUserModal();
});
...
document.getElementById('user-form')?.addEventListener('submit', (e) => {
    ...
    const permissions = {
        ...
        teacher_attendance: formData.get('perm_teacher_attendance') === 'true',
        students: formData.get('perm_students') === 'true',
        fees: formData.get('perm_fees') === 'true',
        exams: formData.get('perm_exams') === 'true',
        reports: formData.get('perm_reports') === 'true',
        users: formData.get('perm_users') === 'true',
        free_students: formData.get('perm_free_students') === 'true',
        private_msg: formData.get('perm_private_msg') === 'true',
        data_mgmt: formData.get('perm_data_mgmt') === 'true',
        messaging: formData.get('perm_messaging') === 'true',
    };
    ...
    if (isEdit) {
        if (Store.updateUser(userData.email, userData)) {
            this.showToast('User updated');
        } else {
            this.showToast('Error updating user');
        }
    } else {
        if (Store.addNewUser(userData)) {
    ...
    this.toggleModal('add-user-modal', false);
    this.refreshCurrentView();
});
...
showLayout() {
    ...
    const userJson = sessionStorage.getItem('dugsiga_user');
    if (!userJson) return; // Should allow login
    ...
    navItems.forEach(item => {
        ...
        // Mapping Views to Permissions
        let requiredPerm = view;
        if (view === 'teacher-attendance') requiredPerm = 'teacher_attendance';
        if (view === 'free-fee-students') requiredPerm = 'free_students';
        if (view === 'parent-messages') requiredPerm = 'private_msg';
        if (view === 'data-management') requiredPerm = 'data_mgmt';
        ...
        // Teacher specific override (if legacy role used without perms)
        if (role === 'teacher') {
            if (view === 'attendance' || view === 'teacher-attendance') {
                item.style.display = 'flex';
            } else {
...
navigateTo(viewName) {
    // Permission Check
    if (this.state.currentUserRole !== 'owner') {
        const userJson = sessionStorage.getItem('dugsiga_user');
        if (userJson) {
            const user = JSON.parse(userJson);
            if (viewName === 'users' && (!user.permissions || !user.permissions.users)) {
                this.showToast('Access Denied');
                return;
            }
            // Add other checks if strictly needed, but Sidebar hiding is primary
        }
    }
    ...
    // Show selected (Dynamically render)
    ...
    switch (viewName) {
        ...
        case 'teacher-attendance': this.renderTeacherAttendance(container); break;
        case 'students': this.renderStudents(container); break;
        case 'teachers': this.renderTeachers(container); break;
        case 'salaries': this.renderSalaries(container); break;
        case 'classes': this.renderClasses(container); break;
        case 'fees': this.renderFees(container); break;
        case 'exams': this.renderExams(container); break;
        case 'reports': this.renderReports(container); break;
        case 'users': this.renderUserManagement(container); break;
        case 'free-fee-students': this.renderFreeStudents(container); break;
        case 'data-management': this.renderDataManagement(container); break;
        case 'messaging': this.renderMessaging(container); break;
        case 'parent-messages': this.renderParentMessages(container); break;
...
renderDashboard(container) {
    ...
    const attendance = Store.getAttendance().filter(a => a.date === new Date().toISOString().split('T')[0]);

    // Request: 120 Students, 5 Teachers
    const graduated = students.filter(s => s.grade === 'Form 4').length;

    // Request: 5 teachers * $250 = $1250
    const teacherCount = teachers.length > 0 ? teachers.length : 5; // Fallback to 5 if empty
    const salaryTotal = teacherCount * 250;

    // Request: 96 Present, 15 Absent, 9 Late (If dynamic matches, great, else logic)
    // We will use real data:
    const present = attendance.filter(a => a.status === 'Present').length;
    const absent = attendance.filter(a => a.status === 'Absent').length;
    const late = attendance.filter(a => a.status === 'Late').length;

    // Financials: Expected = Students * 20
    const payingStudents = students.filter(s => !s.isFree).length;
    const expected = payingStudents * 20;

    // Collected
    const currentYear = Store.state.currentYear;
    const fees = Store.getFees().filter(f => f.year === currentYear);
    const collected = fees.reduce((sum, f) => sum + (f.amountPaid || 0), 0);

    const pending = expected - collected; // Or calculate from Unpaid fees

    container.innerHTML = `
            <div class="stats-grid">
                ${this.createStatCard('Total Students', students.length, 'users', 'blue')}
                ${this.createStatCard('Total Teachers', teacherCount, 'briefcase', 'indigo')}
                ${this.createStatCard('Graduated (Form 4)', graduated, 'award', 'purple')}
                ${this.createStatCard('Teacher Salaries', '$' + salaryTotal.toLocaleString(), 'dollar-sign', 'green')}
            </div>
            ...
            <div class="dashboard-grid-2">
                ...
                <div class="card">
                    ...
                    <div class="attendance-stats" style="display: flex; gap: 20px; text-align: center; justify-content: space-around; padding: 20px 0;">
                        <div>
                            <div class="stat-value text-success" style="font-size: 2rem; font-weight: bold;">${present}</div>
                            <div class="text-sm">Present</div>
                        </div>
                        <div>
                            <div class="stat-value text-danger" style="font-size: 2rem; font-weight: bold;">${absent}</div>
                            <div class="text-sm">Absent</div>
                        </div>
                        <div>
                            <div class="stat-value text-warning" style="font-size: 2rem; font-weight: bold;">${late}</div>
                ...
                <div class="card">
                    ...
                    <div class="finance-list" style="margin-top: 15px;">
                        <div style="display: flex; justify-content: space-between; margin-bottom: 10px; padding: 8px 0; border-bottom: 1px solid #eee;">
                            ...
                            <span class="font-bold">$${expected.toLocaleString()}</span>
                        </div>
                        <div style="display: flex; justify-content: space-between; margin-bottom: 10px; padding: 8px 0; border-bottom: 1px solid #eee;">
                            <span>Collected Revenue</span>
                            <span class="font-bold text-success">$${collected.toLocaleString()}</span>
                        </div>
                         <div style="display: flex; justify-content: space-between; margin-bottom: 10px; padding: 8px 0;">
                            <span>Pending Revenue</span>
                            <span class="font-bold text-danger">$${pending.toLocaleString()}</span>
                        </div>
                    </div>
                </div>
            </div>
             ...
             <div class="card mt-6">
                <h3 class="card-title">Pending Fees (Sample)</h3>
                 <div class="table-responsive">
                    <table class="table">
                        ...
                        <tbody>
                            ${fees.filter(f => f.status === 'UNPAID').slice(0, 5).map(f => {
        const s = Store.getStudent(f.studentId);
        return `
                                <tr>
                                    <td>${s ? s.fullName : 'Unknown'}</td>
                                    <td>${s ? s.grade + ' ' + s.section : '-'}</td>
                                    <td>$${f.amount}</td>
...
openAddTeacherModal() {
    const existing = document.getElementById('teacher-modal');
    if (existing) existing.remove();

    const html = `
            <div id="teacher-modal" class="modal-overlay">
                <div class="modal-content">
                    <h3 class="font-bold text-lg mb-4">Add/Edit Teacher</h3>
                    <form id="teacher-form">
                        <input type="hidden" name="id">
                        <input type="text" name="name" placeholder="Full Name" class="form-input mb-3" required>
                        <input type="text" name="phone" placeholder="Phone Number" class="form-input mb-3" required>
                        <select name="gender" class="form-input mb-3" required>
                            <option value="">Select Gender</option>
                            <option value="Male">Male</option>
                            <option value="Female">Female</option>
                        </select>
                        <input type="number" name="salary" placeholder="Salary ($)" class="form-input mb-3" required>
                        <input type="text" name="subject" placeholder="Main Subject" class="form-input mb-4" required>
                        
                        <div class="flex justify-end gap-2">
                            <button type="button" class="btn btn-ghost" onclick="document.getElementById('teacher-modal').remove()">Cancel</button>
    ...
    document.getElementById('teacher-form').addEventListener('submit', (e) => {
        ...
        const teacher = {
            id: fd.get('id') || 'KT' + Math.floor(Math.random() * 10000), // Check if ID exists for edit
            name: fd.get('name'),
            phone: fd.get('phone'),
        ...
        if (existingId) {
            // Update
            const index = Store.state.teachers.findIndex(t => t.id === existingId);
            if (index >= 0) {
                Store.state.teachers[index] = { ...Store.state.teachers[index], ...teacher };
                Store.saveToStorage();
                App.showToast('Teacher updated');
            }
        } else {
            // Add
            Store.state.teachers.push(teacher);
            Store.saveToStorage();
            App.showToast('Teacher added');
        }
...
openEditTeacherModal(id) {
    ...
    const teacher = Store.getTeachers().find(t => t.id === id);
    if (!teacher) return;

    setTimeout(() => {
        const form = document.getElementById('teacher-form');
        form.id.value = teacher.id;
        form.name.value = teacher.name;
        form.phone.value = teacher.phone;
        form.gender.value = teacher.gender;
        form.salary.value = teacher.salary;
...
container.innerHTML = `
    ...
    renderSalaries(container) {
        ...
        // If state has filtered month, use it, else current
        const currentMonth = this.state.salaryMonth || `${ monthNames[date.getMonth()] } ${ date.getFullYear() } `;

        container.innerHTML = `
    < div class="flex flex-col gap-6 animate-fade-in" >
                <div class="flex justify-between items-center">
                    <div>
                         <h2 class="text-2xl font-bold" style="color: var(--color-primary-text);">Salary Management</h2>
                         <p class="text-secondary-text">Manage teacher salaries and payments</p>
                    </div>
                </div>
                ...
                <div class="card flex gap-4 items-end">
                    <div class="form-group flex-1">
                        <label class="block text-sm font-bold mb-2">Select Month</label>
                        <select id="salary-month-select" class="form-input">
                            ${this.generateMonthOptions(currentMonth)}
                        </select>
                    </div>
                    <button class="btn btn-primary" onclick="App.filterSalaries()">Filter</button>
                </div>

                <div class="card">
                     <div class="flex justify-between items-center mb-4">
                        <h3 class="font-bold text-lg">Teachers Salary - ${currentMonth}</h3>
                        <div class="flex gap-2">
                             <button class="btn btn-outline" onclick="window.print()"><i data-feather="printer"></i> Print</button>
                        </div>
                    </div>
                     <div class="table-container">
                        <table class="table">
                            ...
                            <tbody>
                                ${teachers.map(t => {
                                    ...
                                    const paidRecords = salaries.filter(s => s.teacherId === t.id && s.month === currentMonth);
                                    const paid = paidRecords.reduce((acc, curr) => acc + curr.amount, 0);
                                    ...
                                    if (paid >= salary && salary > 0) {
                                        ...
                                        status = 'Partial';
                                        statusClass = 'status-pending'; // Reusing pending style
                                    }
                                    ...
                                    <tr>
                                        ...
                                        <td class="${balance > 0 ? 'text-red-500 font-bold' : 'text-green-500'}">$${balance.toFixed(2)}</td>
                                        <td><span class="status-badge ${statusClass}">${status}</span></td>
                                        <td>
                                            ${balance > 0 ? `
                                            <button class="btn btn-success btn-sm" onclick="App.openPaySalaryModal('${t.id}', ${balance}, '${currentMonth}')">
                                                <i data-feather="dollar-sign" style="width:14px;"></i> Pay
                                            </button>
                                            ` : '<span class="text-green-600 text-sm"><i data-feather="check"></i> Completed</span>'}
                                        </td>
                                    </tr>
                                    `;
                                }).join('')}
                ...
                <!--Payment History-- >
    <div class="card mt-6">
        <h3 class="font-bold text-lg mb-4">Payment History (${currentMonth})</h3>
        <div class="table-responsive">
            <table class="table">
                <thead>
                    <tr>
                    ...
                    ${salaries.filter(s => s.month === currentMonth).map(s => {
                        ...
                        return `
                                    <tr>
                                        ...
                                        <td class="font-bold text-green-600">+$${s.amount}</td>
                                    </tr>
                                    `;
                    }).join('')}
                    ${salaries.filter(s => s.month === currentMonth).length === 0 ? '<tr><td colspan="4" class="text-center text-gray-400 py-4">No payments recorded this month.</td></tr>' : ''}
                </tbody>
            </table>
        </div>
    </div>
    ...
    generateMonthOptions(selected) {
        ...
        for(let i=0; i<18; i++) {
            ...
            html += `< option value = "${mStr}" ${ mStr === selected ? 'selected' : '' }> ${ mStr }</option > `;
    ...
    filterSalaries() {
        const val = document.getElementById('salary-month-select').value;
        this.state.salaryMonth = val;
        this.refreshCurrentView();
    },

    openPaySalaryModal(tId, balance, month) {
        const t = Store.getTeachers().find(te => te.id === tId);
        if (!t) return;
    ...
    < div id = "pay-modal" class="modal-overlay" >
        <div class="modal-content">
            <h3 class="font-bold text-lg mb-4">Pay Salary - ${t.name}</h3>
            <p class="mb-4 text-sm text-gray-600">Month: <b>${month}</b> | Balance: <b class="text-red-500">$${balance}</b></p>
            <form id="pay-form">
                <label class="block text-sm font-bold mb-2">Amount to Pay</label>
                <input type="number" name="amount" value="${balance}" max="${balance}" class="form-input mb-4" required>

                    <div class="flex justify-end gap-2">
                        <button type="button" class="btn btn-ghost" onclick="document.getElementById('pay-modal').remove()">Cancel</button>
    ...
    `;
        ...
        document.getElementById('pay-form').addEventListener('submit', (e) => {
            ...
            if (amount > 0) {
                ...
                App.showToast(`Paid $${ amount } to ${ t.name } `);
...
renderTeacherAttendance(container) {
    const teachers = Store.getTeachers();
    const today = new Date().toISOString().split('T')[0];
    const att = Store.getTeacherAttendance().filter(a => a.date === today);

    container.innerHTML = `
            <div class="flex flex-col gap-6 animate-fade-in">
                 <div class="flex justify-between items-center">
                    <h2 class="text-2xl font-bold">Teacher Attendance</h2>
                     <div class="text-sm font-medium text-gray-500">${new Date().toDateString()}</div>
                </div>
                ...
                <div class="card">
                     <div class="table-responsive">
                        <table class="table">
                            ...
                            <tbody>
                                ${teachers.map(t => {
        const record = att.find(a => a.teacherId === t.id);
        const status = record ? record.status : 'Present'; // Default
        return `
                                    <tr>
                                        ...
                                        <td>
                                            <div class="flex gap-4">
                                                <label class="flex items-center gap-2 cursor-pointer">
                                                    <input type="radio" name="att_${t.id}" value="Present" ${status === 'Present' ? 'checked' : ''} onchange="Store.recordTeacherAttendance('${t.id}', 'Present', '${today}')">
                                                    <span class="text-success">Present</span>
                                                </label>
                                                <label class="flex items-center gap-2 cursor-pointer">
                                                    <input type="radio" name="att_${t.id}" value="Absent" ${status === 'Absent' ? 'checked' : ''} onchange="Store.recordTeacherAttendance('${t.id}', 'Absent', '${today}')">
                                                    <span class="text-danger">Absent</span>
                                                </label>
                                                <label class="flex items-center gap-2 cursor-pointer">
                                                    <input type="radio" name="att_${t.id}" value="Late" ${status === 'Late' ? 'checked' : ''} onchange="Store.recordTeacherAttendance('${t.id}', 'Late', '${today}')">
    ...
    container.innerHTML = `
             <div class="flex flex-col gap-6 animate-fade-in">
                <div class="flex justify-between items-center">
                    ...
                    <button onclick="App.openAddUserModal()" class="btn btn-primary"><i data-feather="plus"></i> Add User</button>
                </div>
                        ...
                        <div class="card">
                            <div class="flex justify-between items-start mb-4">
                                <div>
                                 ...
                                 <button class="text-red-500 hover:text-red-700" onclick="if(confirm('Delete user?')){ Store.deleteUser('${u.email}'); App.refreshCurrentView(); }">
                                    <i data-feather="trash-2" style="width:16px;"></i>
                                </button>
                            </div>
                            <div class="mb-4">
                                <p class="text-sm"><strong>Email:</strong> ${u.email}</p>
                                <p class="text-sm"><strong>Password:</strong> ${u.password}</p>
                            </div>
                            <div>
                                <h4 class="text-xs font-bold uppercase text-gray-400 mb-2">Permissions</h4>
                                <div class="flex flex-wrap gap-2">
                                    ${Object.entries(u.permissions || {}).filter(([k, v]) => v).map(([k, v]) => `
                                        <span class="badge badge-neutral text-xs">${k}</span>
                                    `).join('')}
                                </div>
                                <button class="btn btn-outline btn-sm w-full mt-4" onclick="App.openEditUserModal('${u.email}')">Edit Access</button>
                            </div>
                        </div>
                    `).join('')}
                </div>
...
openAddUserModal() {
    // Simple prompt for now or full modal. Let's do a full modal HTML injection.
    // For brevity in this replace, I will call a helper separate or inject here.
    // Injecting simple modal:
    ...
    const html = `
            <div id="user-modal" class="modal-overlay">
                <div class="modal-content">
                    <h3 class="font-bold text-lg mb-4">Add/Edit User</h3>
                    <form id="user-form">
                        <input type="email" name="email" placeholder="Email" class="form-input mb-3" required>
                        <input type="text" name="name" placeholder="Name" class="form-input mb-3" required>
                        <input type="password" name="password" placeholder="Password" class="form-input mb-3" required>
                        <select name="role" class="form-input mb-4">
                            <option value="teacher">Teacher</option>
                            <option value="admin">Admin</option>
                            <option value="fees">Fees Officer</option>
                        </select>
                        <div class="mb-4">
                            <label class="block text-sm font-bold mb-2">Access Rights</label>
                            <div class="grid grid-cols-2 gap-2 text-sm">
                                ${['dashboard', 'attendance', 'teacher_attendance', 'students', 'teachers', 'fees', 'parents', 'exams', 'reports', 'users', 'free_students', 'private_msg', 'data_mgmt', 'messaging'].map(p => `
                                    <label class="flex items-center gap-2"><input type="checkbox" name="perm_${p}" value="true"> ${p.replace('_', ' ')}</label>
                                `).join('')}
                            </div>
                        </div>
                        <button type="submit" class="btn btn-primary w-full">Save User</button>
                    </form>
                    <button onclick="document.getElementById('user-modal').remove()" class="absolute top-4 right-4"><i data-feather="x"></i></button>
                </div>
            </div>
        `;
    document.body.insertAdjacentHTML('beforeend', html);
    ...
    document.getElementById('user-form').addEventListener('submit', (e) => {
        ...
        const fd = new FormData(e.target);
        const permissions = {};
        ['dashboard', 'attendance', 'teacher_attendance', 'students', 'teachers', 'fees', 'parents', 'exams', 'reports', 'users', 'free_students', 'private_msg', 'data_mgmt', 'messaging'].forEach(p => {
            permissions[p] = fd.get(`perm_${p}`) === 'true'; // Checkbox value
        });
        ...
        if (Store.state.users.find(u => u.email === user.email)) {
            Store.updateUser(user.email, user);
        } else {
...
openEditUserModal(email) {
    ...
    setTimeout(() => {
        ...
        Object.entries(user.permissions).forEach(([k, v]) => {
            if (form[`perm_${k}`]) form[`perm_${k}`].checked = v;
        });
    }, 100);
},

// --- Exams View (New Folder Structure) ---
renderExams(container) {
    // If drilling down
    if (this.state.currentExamsGrade) {
        if (this.state.currentExamsSection) {
            this.renderExamStudentsList(container);
        } else {
    ...
    container.innerHTML = `
            <div class="flex flex-col gap-6 animate-fade-in">
                 <h2 class="text-2xl font-bold">Exams - Select Form</h2>
                 <div class="grid grid-cols-2 md:grid-cols-4 gap-6">
                    ${['Form 1', 'Form 2', 'Form 3', 'Form 4'].map(form => `
                        <div class="card cursor-pointer hover:shadow-lg transition-all" onclick="App.openExamsGrade('${form}')">
                            <div class="flex flex-col items-center justify-center py-8">
                                <i data-feather="folder" style="width: 48px; height: 48px; color: #3b82f6; fill: #dbeafe;"></i>
                                <span class="mt-4 font-bold text-lg">${form}</span>
                            </div>
                        </div>
                    `).join('')}
                 </div>
    ...
    container.innerHTML = `
            <div class="flex flex-col gap-6 animate-fade-in">
                 <div class="flex items-center gap-4">
                    <button onclick="App.state.currentExamsGrade=null; App.refreshCurrentView()" class="btn btn-ghost"><i data-feather="arrow-left"></i> Back</button>
                    <h2 class="text-2xl font-bold">${this.state.currentExamsGrade} - Select Section</h2>
                 </div>
                 <div class="grid grid-cols-2 md:grid-cols-4 gap-6">
                    ${['A', 'B'].map(sec => `
                        <div class="card cursor-pointer hover:shadow-lg transition-all" onclick="App.openExamsSection('${sec}')">
                            <div class="flex flex-col items-center justify-center py-8">
                                <i data-feather="users" style="width: 48px; height: 48px; color: #10b981; fill: #d1fae5;"></i>
                                <span class="mt-4 font-bold text-lg">Section ${sec}</span>
                            </div>
                        </div>
                    `).join('')}
                 </div>
...
renderExamStudentsList(container) {
    const students = Store.getStudents().filter(s =>
        s.grade === this.state.currentExamsGrade &&
        s.section === this.state.currentExamsSection
    );

    container.innerHTML = `
             <div class="flex flex-col gap-6 animate-fade-in">
                 <div class="flex items-center gap-4">
                    <button onclick="App.state.currentExamsSection=null; App.refreshCurrentView()" class="btn btn-ghost"><i data-feather="arrow-left"></i> Back</button>
                    <h2 class="text-2xl font-bold">${this.state.currentExamsGrade} ${this.state.currentExamsSection} - Students</h2>
                 </div>
                            ...
                            <tbody>
                                ${students.map((s, idx) => `
                                    <tr>
                                        ...
                                        <td>
                                            <button class="btn btn-primary btn-sm" onclick="App.openStudentExamMarks('${s.id}')">
                                                <i data-feather="edit-3" style="width:14px;"></i> View/Edit Marks
                                            </button>
                                        </td>
                                    </tr>
                                `).join('')}
...
openStudentExamMarks(studentId) {
    const student = Store.getStudent(studentId);
    // Only show modal for 11 subjects
    const subjects = [
        "Math", "English", "History", "Physics", "Arabic", "Tarbiya",
        "Information Technology", "Af-Somali", "Chemistry", "Geography", "Biology"
    ];

    const term = "Term 1"; // Could be selectable

    const existing = document.getElementById('marks-modal');
    if (existing) existing.remove();

    const marks = Store.getStudentMarks ? Store.getStudentMarks(studentId, term) : [];
    // Note: I need to ensure getStudentMarks exists in Store, I added it in previous step.

    const html = `
             <div id="marks-modal" class="modal-overlay">
                <div class="modal-content" style="max-width: 600px;">
                    <h3 class="font-bold text-lg mb-4">Marks for ${student.fullName} (${term})</h3>
                    <form id="marks-form" class="grid grid-cols-2 gap-4">
                        ${subjects.map(sub => {
        const found = marks.find(m => m.subject === sub);
        const val = found ? found.mark : '';
        return `
                                <div>
                                    <label class="block text-xs font-bold uppercase text-gray-500 mb-1">${sub}</label>
                                    <input type="number" name="subj_${sub}" value="${val}" class="form-input" placeholder="0-100">
                                </div>
                            `;
    }).join('')}
                        <input type="hidden" name="studentId" value="${studentId}">
                        <div class="col-span-2 mt-4 flex justify-end gap-2">
                            <button type="button" class="btn btn-ghost" onclick="document.getElementById('marks-modal').remove()">Close</button>
    ...
    document.getElementById('marks-form').addEventListener('submit', (e) => {
        ...
        subjects.forEach(sub => {
            const val = fd.get(`subj_${sub}`);
            if (val !== '') {
                Store.saveStudentMark(sId, sub, val, term);
            }
        });

        App.showToast('Marks saved successfully');
        document.getElementById('marks-modal').remove();
    });
},
